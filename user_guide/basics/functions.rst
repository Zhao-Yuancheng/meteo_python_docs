...
按值／按引用传递：小心可变参数的「后座力」
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

参数传进去之后，能不能在函数里动到原来的数据，取决于传的\ **是什么类型**：

- **不可变对象**（数字、字符串、元组）：按「值」传。函数里怎么改形参，都不影响调用处的原对象；
- **可变对象**（列表、字典、集合）：按「引用」传。函数里改列表、改字典键值，会\ **直接改到调用处的原对象**，函数结束后改动依然在。

.. code-block:: python

   def add_todays_max(temps):          # 想给气温序列补一个「今日最高」
       temps.append(max(temps))        # 直接改参数列表

   daily = [18, 22, 25]
   add_todays_max(daily)
   print(daily)                        # [18, 22, 25, 25] —— 原列表被改了

这通常是\ **好事**（省内存、批量就地处理），但要是函数只想「算一算、不动原件」，就会被偷改的数据坑到。解法是先复制一份再算：

.. code-block:: python

   def add_todays_max(temps):
       result = temps.copy()           # 复制一份，原件无损
       result.append(max(result))
       return result

   daily = [18, 22, 25]
   out = add_todays_max(daily)
   print(daily)                        # [18, 22, 25]
   print(out)                          # [18, 22, 25, 25]

这与第 6 章「NumPy 切片返回视图、改动会连带原数组」是同一类思维——想保护原件，先分清「改对象」和「换名字」，必要时用 copy() 隔离。
...
基于 4.7 的闭包，能拼出一个流行特性：**装饰器**。它用 @ 语法，把原始函数传进一个包装函数，用包装后的版本替换掉原始函数——简洁地给函数「套上一层外套」，而不用改动函数体本身。
...
.. code-block:: python
   ...
   def log_time(func):
       def wrapper(*args, **kwargs):
           t0 = time.time()
           result = func(*args, **kwargs)     # 先跑原函数
           print(f"{func.__name__} 耗时 {time.time() - t0:.3f} s")
           return result
       return wrapper

   @log_time
   def load_station_temps():
       return list(range(1_000_000))

   load_station_temps()                        # load_station_temps 耗时 0.013 s

这里 ``@log_time`` 相当于写下了 ``load_station_temps = log_time(load_station_temps)``：函数名被替换成包了一层计时的版本，原函数体一行没动。气象批处理里，给「读数据、算统计、出图」各步都裹一层计时／打日志／补缺测的外衣，就能做到改一处、处处生效。
...
.. seealso:: 配套练习：:doc:`/tutorials/basics/ch04_practice`　·　术语参考：:doc:`/api/basics/ch04_terms`　·　示例画廊 :doc:`/gallery/plot_basics/index`。