...
本动画把本节的数据类型核心概念演给你看：① 变量是「贴标签」——内存盒子先存在，变量名标签贴上去；② int 与 float 的精度差异（0.1 + 0.2 != 0.3 的可视化）；③ 列表 / 元组 / 字典三种容器类比（抽屉、密封盒、标签柜）。点击播放，可暂停拖动进度条反复观看。

.. video:: /_static/videos/T202_Python基础概念动画演示_av1.webm
   :width: 100%
...
.. seealso:: 名词详解见 :doc:`/api/basics/ch02_terms`　·　相关词条：:term:`变量`　·　:term:`可变类型`　·　:term:`不可变类型`。
...
有些字符不能直接写进字符串，需要用反斜杠 \\ 转义——它告诉解释器「后面这个字符，我指的是字符本身，而不是它的语法含义」。气象文本里最常见的是换行与制表：

.. code-block:: python

   print("2024-07-15\n14:00")     # \n 换行：日期和时刻各占一行
   print("晴\t多云\t扬沙")          # \t 制表：排成对齐的一列
   print("他说：\"高温\"")          # \" 双引号：在字符串里嵌一个引号

常用的转义符：\\n 换行、\\t 制表、\\\\ 反斜杠本身、\\" 双引号、\\' 单引号。读 CSV 观测文件、处理带换行的多行文本时，\\n 常会悄悄混进值里，把它识别出来并替换干净，是数据清洗的入门功课。
...
Python 内置了一批「开箱即用」的函数，配合运算符能完成完整的气象数值变换：
...
.. code-block:: python
   ...
   print(abs(-2.1))           # 2.1
   print(round(12.765, 1))    # 12.8
   print(divmod(730, 24))     # (30, 10)
   print(chr(9731))           # ☃ 雪花字符
   print(ord("清"))           # 28165

.. warning::

   eval() 可以把字符串当表达式执行（如 eval("2+3") 得 5），但它能运行任意代码，**绝不用于解析用户输入的数值**。气象里读入的字符串一律用 float() / int() 转型，别为图省事用 eval。
...
.. seealso::
   ...
   - 术语详解：:doc:`/api/basics/ch02_terms` —— 每个名词的生活类比与易混淆点；
