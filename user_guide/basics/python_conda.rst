...
..      </video>
..      <p class="ch-video-caption">视频 1-1 · Miniconda 安装与 PATH 配置</p>
..    </div>
...
..        您的浏览器不支持 HTML5 视频，请下载观看。
..      </video>
..      <p class="ch-video-caption">视频 1-2 · VS Code 配置与 Python 扩展</p>
..    </div>
...
..        您的浏览器不支持 HTML5 视频，请下载观看。
..      </video>
..      <p class="ch-video-caption">视频 1-3 · Python 代码执行流程动画</p>
..    </div>
...
.. note::
   ...
   Python 既不是「纯解释型」也不是「纯编译型」：它先把整个源文件编译成字节码，再逐条解释执行字节码。术语细节见 :doc:`/api/basics/ch01_terms` 中的「解释器」词条。
...
每个程序都有「进」和「出」：\ **输出**\把结果写给用户，\ **输入**\从用户那里收数据。刚跑通的 main.py 其实是把「出」用上了（print），这里把这两件事一次讲透，顺便为第 2、3 章频繁出现的 ``float(input(...))`` 埋下伏笔。

print()：把结果写给屏幕
~~~~~~~~~~~~~~~~~~~~~~~

``print()`` 把任意内容输出到屏幕，可一次传多个参数，用空格（或 sep）分隔；结尾默认换行，可用 end= 换成别的：

.. code-block:: python

   print("兰州站", "海拔", 1517.2)       # 兰州站 海拔 1517.2
   print("今日", end=" ")
   print("晴")                           # 今日 晴（end 让两行接在一起）

input()：从键盘收数据
~~~~~~~~~~~~~~~~~~~~~~

``input()`` 等待用户从键盘输入，**返回的一定是字符串**（string），哪怕你输入的是 36.5：

.. code-block:: python

   t = input("请输入气温(℃)：")      # 用户输入 36.5
   print(type(t))                    # <class 'str'>，不是 float！

.. warning::

   ``input()`` 永远返回字符串。要做算术必须先用 ``int()`` / ``float()`` 转成数值，这也是第 3 章 ``float(input("..."))`` 写在一行的原因——一边收数据，一边顺手转型。
...
.. seealso:: 术语详解见 :doc:`/api/basics/ch01_terms`——解释器、环境、PATH、包管理器、IDE、脚本等词条，每个都配有「一句话定义 + 生活类比 + 代码示例 + 易混淆点」。配套练习见 :doc:`/tutorials/basics/ch01_practice`。
