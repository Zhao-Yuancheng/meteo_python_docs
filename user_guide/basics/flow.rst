...
程序跑着跑着，冷不丁甩出一串红色报错——刚才的 ``float(input(...))`` 里，用户要是敲了 abc 而不是数字，程序当场就崩。这类「运行时才暴露的非法动作」就是\ **异常**（Exception）。成熟的做法不是躲开错误，而是提前想好「这一步可能出错，出错就绕道走」。

try / except：接住异常
~~~~~~~~~~~~~~~~~~~~~~~

用 ``try`` 包住可能出错的代码，再用 ``except`` 接住错误并给出兜底处理：
...
这样程序不会崩溃，而是友好地提示后继续运行。

.. note::

   ``except`` 可以写多个，各自接住不同异常；不写类型则接住所有异常。气象数据处理里建议「先细后粗」：先接住具体的 ``ValueError``、``FileNotFoundError`` 这类类型错误做定制处理，最后再加一个不带类型的 ``except`` 兜住漏网的。全程不接异常、听任它崩溃，是排错期最不可取的写法。

else 与 finally
""""""""""""""""

完整的 ``try`` 还能带上 ``else`` 与 ``finally``：
...
* ``else`` 只有在 ``try`` 没出错时才执行；
* ``finally`` 无论成败都执行，最适合「关闭文件、释放连接」这类收尾动作。

读取气象 CSV 的规范写法，正好把三样都用上：

.. code-block:: python

   try:
       f = open("station_temps.csv", "r")   # r 表示只读
...
Python 报错会给三样线索：① 异常类型（``ValueError``、``NameError``…）；② 异常内容提示（如 ``name 'abc' is not defined``）；③ 出错代码所在文件与行号。排错时按「类型 → 信息 → 行号」的顺序逐条看，往往一眼就能定位问题。

文件操作
--------

气象业务里，数据一半躺在文件里——CSV、txt、NetCDF……学会读写文件，才算摸到「真数据」的门槛。
...
.. code-block:: python
   ...
   f = open("station_temps.csv", "r")   # 打开：r 只读
   text = f.read()                      # 操作：读
   f.close()                            # 关闭

``open()`` 的第二个参数是「打开模式」，常用几种：
...
.. list-table::
   ...
   * - 模式
     ...
     - 典型用途
   * - ``r``
     - 只读（默认），文件不存在报 ``FileNotFoundError``
     - 读数据文件
   * - ``w``
     - 覆盖写，若文件已存在会清空重写；不存在则新建
     - 导出处理结果
   * - ``a``
     - 追加写，在文件末尾续写，不清空原有内容
     - 逐批追加日志、观测记录
   * - ``t`` / ``b``
     - 文本模式 / 二进制模式，可组合成 ``rt``、``wb`` 等
     - 图片、NetCDF 等二进制用 ``b``，文本用 ``t``

读文件用 ``read()``（全读）、``readline()``（读一行）、``readlines()``（读成行的列表）；写文件用 ``write()`` / ``writelines()``。

with：自动关闭的更优解
~~~~~~~~~~~~~~~~~~~~~~~

手写 ``f.close()`` 有个隐患——要是中途出错，文件可能没被关闭，句柄一直占着。更稳的写法是 ``with``：

.. code-block:: python

   with open("station_temps.csv", "r") as f:
       content = f.read()
       print(content)
   # with 块结束，文件自动关闭，无需手动 close()

块内代码执行完（无论是否抛异常），文件都会被自动关闭。这也是后续第 6–8 章读 CSV、NetCDF 时最常见的写法。

.. tip::

   结合上一节，读文件最稳的完整写法是 ``with`` 包住 ``try``、``except`` 接住 ``FileNotFoundError``，再把读到的行用 ``float()`` / ``int()`` 逐列转换——文件、异常两把刀一起用，才算搭好「读气象原始数据」的地基。
...
.. seealso::
   ...
   - 术语详解：:doc:`/api/basics/ch03_terms` —— 每个名词的生活类比与易混淆点；
