...
8.4.3 条件掩膜：where 按条件保留或替换
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

气象业务常要「只保留满足条件的格点，其余隐藏」。``数据.where(条件, 其他值)`` 正为此设计：满足条件处保留原值，不满足处换成 NaN（或你指定的值）。

.. code-block:: python

   temp = ds["气温"]
   temp.where(temp > 35)                  # 只保留 >35℃ 的格点，其余置 NaN

   # 距平绝对值超过 ±1℃ 的格点打上掩膜（极值显著性打点前的常见处理）
   anomaly = temp - temp.mean("时间")
   strong = anomaly.where(abs(anomaly) > 1)

   # 不满足条件时改成指定值，而不是 NaN
   temp.where(temp > 0, other=0)

.. warning::

   where 返回的仍是 Xarray 对象，只是把不满足条件的格点换成了 NaN——**维度大小一个不少**。若要把这些 NaN 真从数据里剔除（只留下满足条件的点），再一步 .to_dataframe().dropna() 去 NaN 行即可。

8.4.4 差分计算：diff 沿维度求变化
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

数学计算里常用「差分」估求变化率与趋势。``数据.diff(维度名)`` 沿该维度返回相邻格点之差：

.. code-block:: python

   temp.diff("time")          # 逐时间步的气温变化量（后一时刻 − 前一时刻）
   temp.diff("lat")           # 沿纬度向的梯度雏形

.. code-block:: python

   dt = temp.diff("time")           # 每相邻两天气温的变化幅度
   warming = (dt > 0).sum("time")   # 升温天数占比（符号统计）

.. note::

   diff 会让该维度\ **少一格**（长度 − 1），因为相邻差只有 n−1 个。若后续要与原数据按时间对齐，可用 assign_coords(time=temp.time[1:]) 之类把时间坐标补对齐。
...
.. seealso:: 术语参考：:doc:`/api/data/ch08_terms`　·　示例画廊 :doc:`/gallery/plot_numpy/index`。