...
7.3.4 缺失值处理：dropna 删除、fillna 填充
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

气象记录常带缺测，读到 DataFrame 里就成了 NaN。多数情况下「先保留 NaN」（让它自然不参与均值、被布尔条件过滤）就够了；但当缺测分布让后续步骤无法进行，就要显式「删」或「填」。

**删除**：dropna 丢掉含 NaN 的行或列。

.. code-block:: python

   df_weather.dropna()                        # 删除含任何 NaN 的行
   df_weather.dropna(subset=["气温"])         # 只看「气温」这一列，有缺测才删行
   df_weather.dropna(axis=1)                  # 删除含 NaN 的列
   df_weather.dropna(how="all")               # 只删整行全为 NaN 的行

**填充**：fillna 把 NaN 换成某个值，气象里最常用的是前向填充（缺口用前一条有效记录顶上）。

.. code-block:: python

   df_weather["气温"].fillna(0)                                        # 填 0（慎用：0℃ 是真实气温）
   df_weather["气温"].fillna(df_weather["气温"].mean())                # 填同列均值
   df_weather["气温"].ffill()                                          # 前向填充：用上一个有效值
   df_weather["气温"].bfill()                                          # 后向填充：用下一个有效值

.. warning::

   填充要\ **谨慎**：把缺测填成 0 或均值，等于「发明了观测数据」，会悄悄扭曲均值、极值统计。气象铁律仍是缺测优先保留 NaN，按业务判断哪些记录不可用；只有时间序列分析（rolling 滑动、插值）等确需连续数据的场景，才用 ffill 补缺口，并在结果的备注里注明。

7.3.5 字符串操作：Series.str 整列清洗
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

第 2 章学的 .strip() / .split() / .replace() 一次只能处理一个字符串。站名、天气现象、日期串这类文本列动辄成千上万行，Pandas 提供了 .str 前缀，把同一套字符串方法\ **向量化**\地作用到整列：

.. code-block:: python

   df_weather["站名"].str.strip()               # 去掉首尾空白
   df_weather["站名"].str.upper()               # 转大写
   df_weather["日期"].str.split("-")            # 按「-」拆成 [年,月,日]
   df_weather["天气"].str.contains("雨")        # 哪些天含「雨」
   df_weather["天气"].str.startswith("晴")      # 以「晴」开头
   df_weather["站名"].str.replace("站", "")     # 替换子串

.. code-block:: python

   df_weather["观测时间"] = pd.to_datetime(
       df_weather["观测时间"].str.replace("/", "-")   # 先整列统一日期分隔符再转时间
   )

.. tip::

   .str 方法还支持\ **正则表达式**：df_weather["天气"].str.contains("暴雨|大暴雨") 能一次匹配多种天气现象，做文本归类、要素提取非常高效。注意 .str 返回的是新列，不会就地改名，要写回就显式赋值。
